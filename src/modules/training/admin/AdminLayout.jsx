...
const AdminLayout = () => {
    ...
    React.useEffect(() => {
        // Fetch all users and filter unread count client-side to avoid index requirement
    ...
    return (
        <div className="flex flex-col lg:flex-row h-screen bg-slate-50 pt-[env(safe-area-inset-top)] lg:pt-0">
                ...
                <div className="flex gap-2">
                    <NotificationBell
                        ...
                        onNotificationClick={(noti) => {
                            if (noti.athleteId) {
                                navigate(`/training/admin/users?athleteId=${noti.athleteId}`);
                            }
                        }}
                    />
                    <button
            ...
            <nav className={`lg:hidden fixed bottom-0 left-0 right-0 bg-slate-900 text-white flex justify-around p-2 z-50 border-t border-slate-800 transition-all ${isMobileLandscape ? 'p-1' : 'p-2'}`}>
...
const AdminLink = ({ to, icon: Icon, label, end = false }) => (
    <NavLink
        to={to}
        end={end}
        className={({ isActive }) =>
...
const MobileLink = ({ to, icon: Icon, label, isMobileLandscape, end = false }) => (
    <NavLink
        to={to}
        end={end}
        className={({ isActive }) =>
