...
const AdminLayout = () => {
    ...
    return (
        <div className="flex flex-col lg:flex-row h-screen bg-slate-50">
            ...
            <div className="lg:hidden bg-slate-900 text-white p-3 flex justify-between items-center shrink-0 border-b border-slate-800">
                <div className="flex items-center gap-2">
                    <span className="font-black text-emerald-400 tracking-wider text-sm">PDP ADMIN</span>
                </div>
                <NavLink to="/training" className="text-xs bg-slate-800 hover:bg-slate-700 px-3 py-1.5 rounded-full text-slate-300 font-bold transition-colors">
                    Salir
                </NavLink>
            </div>

            {/* Main Content */}
            <main className="flex-1 overflow-y-auto p-3 lg:p-6 relative z-0 pb-20 lg:pb-6">
                <Outlet />
            </main>

            {/* Mobile Bottom Navigation (Hidden on Desktop) */}
            <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-slate-900 text-white flex justify-around p-2 pb-safe z-50 border-t border-slate-800">
                <MobileLink to="/training/admin/global-creator" icon={Sparkles} label="Editor" />
                <MobileLink to="/training/admin/programs" icon={Calendar} label="Progr." />
                <MobileLink to="/training/admin/users" icon={Users} label="Usuarios" />
            </nav>
        </div>
    );
};
...
const MobileLink = ({ to, icon: Icon, label }) => (
    <NavLink
        to={to}
        className={({ isActive }) =>
            `flex flex-col items-center justify-center gap-0.5 p-1.5 rounded-lg min-w-[55px]
