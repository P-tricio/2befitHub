...
const Login = () => {
    ...
    const translateError = (err) => {
        ...
        return `Error: ${err.message || 'Ha ocurrido un problema. Inténtalo de nuevo.'}`;
    };
    ...
    return (
        <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
            ...
            >
                ...
                <div className="bg-slate-900 p-8 text-center relative overflow-hidden">
                    ...
                    <p className="text-slate-400 text-sm">
                        {isLogin ? 'Acceso a Plataforma de Entrenamiento' : 'Crea tu cuenta de entrenamiento hoy'}
                    </p>
                </div>

                {/* Tab Switcher */}
                <div className="flex p-2 bg-slate-100 mx-8 mt-6 rounded-2xl relative">
                    <div
                        className={`absolute top-2 bottom-2 w-[calc(50%-8px)] bg-white rounded-xl shadow-sm transition-all duration-300 ease-out z-0 ${isLogin ? 'left-2' : 'left-[calc(50%)]'}`}
                    />
                    <button
                        onClick={() => { setIsLogin(true); setError(''); setSuccess(''); }}
                        className={`flex-1 py-2 text-sm font-bold relative z-10 transition-colors ${isLogin ? 'text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Entrar
                    </button>
                    <button
                    ...
                    <AnimatePresence mode="wait">
                        {error && (
                            <motion.div
                                ...
                                className="bg-red-50 text-red-600 p-4 rounded-xl text-sm font-bold flex items-center gap-3 overflow-hidden"
                            >
                        ...
                        {success && (
                            <motion.div
                                ...
                                animate={{ opacity: 1, height: 'auto', marginBottom: 24 }}
                                exit={{ opacity: 0, height: 0, marginBottom: 0 }}
                                className="bg-emerald-50 text-emerald-600 p-4 rounded-xl text-sm font-bold flex items-center gap-3 overflow-hidden"
                            >
